...
static LRESULT CALLBACK tooltip_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		SetWindowLongPtr(hWnd, GWLP_USERDATA, (LPARAM)ti);
		break;
	...
	case WM_DESTROY:
		if ((ti = (TOOLTIP_INFO *)GetWindowLongPtr(hWnd, GWLP_USERDATA)) != NULL) {
			if (ti->hfont != NULL) {
	...
	case WM_TOOLTIP_HIDE:
		...
		if ((ti = (TOOLTIP_INFO *)GetWindowLongPtr(hWnd, GWLP_USERDATA)) != NULL && ti->buf != NULL) {
			mem_free(&ti->buf);
		}
		break;

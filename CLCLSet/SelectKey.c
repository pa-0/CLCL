...
static LRESULT CALLBACK selectkey_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		SetWindowLongPtr(hWnd, GWLP_USERDATA, (LPARAM)si);
		break;
	...
	case WM_DESTROY:
		if ((si = (SELECTKEY_INFO *)GetWindowLongPtr(hWnd, GWLP_USERDATA)) != NULL) {
#ifdef OP_XP_STYLE
			// XP
			if (si->hTheme != NULL) {
...
#ifdef OP_XP_STYLE
	case WM_NCPAINT:
		if ((si = (SELECTKEY_INFO *)GetWindowLongPtr(hWnd, GWLP_USERDATA)) == NULL || si->hTheme == NULL) {
			return DefWindowProc(hWnd, msg, wParam, lParam);
		}
		// XP用の背景描画
	...
	case WM_THEMECHANGED:
		if ((si = (SELECTKEY_INFO *)GetWindowLongPtr(hWnd, GWLP_USERDATA)) == NULL || si->hModThemes == NULL) {
			break;
		}
		// XPテーマの変更
		if (si->hTheme != NULL) {
